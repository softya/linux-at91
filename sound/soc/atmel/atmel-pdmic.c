...
static struct atmel_pdmic_pdata *atmel_pdmic_dt_init(struct device *dev)
{
	...
	if (of_property_read_u32(np, "atmel,mic-offset", (u32 *)&pdata->mic_offset))
		pdata->mic_offset = 0;
...
static irqreturn_t atmel_pdmic_interrupt(int irq, void *dev_id)
{
	...
	if (pdmic_isr & PDMIC_ISR_OVRE) {
		...
		snd_pcm_stream_lock_irqsave(dd->substream, flags);
		if (snd_pcm_running(dd->substream))
			snd_pcm_stop(dd->substream, SNDRV_PCM_STATE_XRUN);
		snd_pcm_stream_unlock_irqrestore(dd->substream, flags);
