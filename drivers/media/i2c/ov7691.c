...
#include <media/videodev2_brcm.h>
#include <linux/gpio.h>
#include <media/ov7691.h>
...
static inline struct ov7692 *to_state(struct v4l2_subdev *sd)
{
	return container_of(sd, struct ov7692, subdev);
}
//
 ...
 */
static int ov7692_read_i2c(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
static int ov7692_write_i2c(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct i2c_msg msg;
...
static int ov7692_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *value)
{
	struct ov7692 *info = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);


	int ret;
	if (info->use_smbus)
		ret =  ov7692_read_smbus(client, reg, value);
	else
		ret =  ov7692_read_i2c(sd, reg, value);
...
static int ov7692_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct ov7692 *info = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	printk("\t%s() [ %.2X, %.2X ],\n", __func__, reg, value);
	if (info->use_smbus)
		return ov7692_write_smbus(client, reg, value);
	else
		return ov7692_write_i2c(sd, reg, value);
}

static int ov7692_write_mask(struct v4l2_subdev *sd, unsigned char reg, unsigned char value, unsigned char mask)
{
//	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	ret = ov7692_read(sd, reg, &curr_value);
	if (ret < 0) return ret;
	curr_value = (unsigned char) curr_value & (~mask);
	curr_value |= value & mask;

	return ov7692_write(sd, reg, (unsigned char) curr_value);
}

/*
 * Write a list of register settings; ff/ff stops the process.
 */
static int ov7692_write_array(struct v4l2_subdev *sd, const struct ov7692_reg vals[])
{
	printk("%s() <<<<<<<<<<<<<<<<<< \n\n", __func__);
	...
	while (vals->reg != 0xff || vals->val != 0xff) {
		int ret;
		if (vals->mask)
			ret=ov7692_write_mask(sd, vals->reg, vals->val, vals->mask);
		else
			ret = ov7692_write(sd, vals->reg, vals->val);
		if (ret < 0)
			return ret;
		vals++;
	}
...
static int ov7692_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	/*ret = ov7692_write_array(sd, configscript_common1);*/
//	if (initneeded > 0) {
//		ret = ov7692_write_array(sd, hawaii_init_common);
//		initneeded = 0;
//	}
			     ...
			     struct v4l2_dbg_register *reg)
{
	...
	if (reg->match.addr != client->addr)
		return -ENODEV;
	reg->size = 2;
	if (ov7692_read(sd, reg->reg, &reg->val))
		return -EIO return 0;
}
			     ...
			     struct v4l2_dbg_register *reg)
{
	...
	if (reg->match.addr != client->addr)
		return -ENODEV;
	if (ov7692_write(sd, reg->reg, reg->val))
		return -EIO;
	return 0;
}
#endif
...
static int ov7692_init_regs(struct v4l2_subdev *sd, u32 val)
{
	unsigned char value = -1, test_reg = 0x82;
	int res;

	msleep(10);
	ov7692_read(sd, test_reg, &value);
	printk("%s BEFORE test register %.2X = %.2X\n", __func__, test_reg, value);

	res = ov7692_write_array(sd, configscript_common1 /*ov7690_default_samsung*/);
...
//	ret = ov7692_init_regs(subdev, 0);
//	if (ret < 0)
//		return ret;

	ret = ov7692_read(subdev, 0x1c, &value);
	pr_info("%s: REG_MIDH = 0x%x (ret=%d)\n", __func__, value, ret);
	if (ret < 0)
		goto ei2c;
	...
	ret = ov7692_read(subdev, 0x1d, &value);
	pr_info("%s: REG_MIDL = 0x%x (ret=%d)\n", __func__, value, ret);
	if (ret < 0)
		goto ei2c;
	if (value != 0xa2) {
		ret = -ENODEV;
	 ...
	 */
	ret = ov7692_read(subdev, 0x0a, &value);
	...
	ret = ov7692_read(subdev, 0x0b, &value);
	pr_info("%s: REG_VER = 0x%x\n", __func__, value);
	if (ret < 0)
		goto ei2c;
	if (value != 0x91) { /* PID + VER = 0x76 / 0x92 */
		ret = -ENODEV;
		goto ei2c;
	}
...
static const struct of_device_id ov7692_of_match[] = {
	{ .compatible = "ovti,ov7692", },
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, ov7692_of_match);
#endif
...
module_i2c_driver(ov7692_i2c_driver);

MODULE_DESCRIPTION("OmniVision OV7692 Camera driver");
MODULE_AUTHOR("Yechiel Cohen <yechielco@embedded-softya.com>");
